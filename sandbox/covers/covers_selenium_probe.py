...
def main():
    ...
    try:
        ...
        result["calendar_discovery_initial"] = calendar

        # The calendar may expose a Week 9 link for the currently selected
        # season even while the requested historical season is only present
        # in the season list. First navigate to the actual 2019-2020 season
        # href, then resolve Week 9 again in that season's context.
        # Covers may render the season selector as a non-anchor control
        # depending on the current client-side state. Prefer the discovered
        # href when available; otherwise interact with the visible
        # 2019-2020 control and let the site perform its own navigation.
        season_url = None

        if calendar["season_link"]:
            season_url = calendar["season_link"]["href"]
            print("2019-2020 season URL discovered:", season_url)
            open_url_and_wait(
                driver,
                season_url,
                wait_seconds,
            ...
            print(
                "No 2019-2020 <a> found; searching rendered controls "
                "for a visible 2019-2020 season selector."
            )
            ...
            for selector in (
                ...
                for element in driver.find_elements(
                    ...
                    try:
                        text = clean(element.text)
                        if text == "2019-2020" and visible(element):
                            season_controls.append(element)
                    except Exception:
                        pass

            clicked = False
            for element in season_controls:
                try:
                    driver.execute_script(
                        "arguments[0].scrollIntoView({block:'center'});",
                        element,
                    ...
                    print(
                        "Clicked 2019-2020 season control; "
                        f"resulting URL: {season_url}"
                    )
                    break
                except Exception as exc:
                    print(f"Season control click attempt: {exc}")

            if not clicked:
                raise RuntimeError(
                    "Could not find a rendered 2019-2020 season "
                    "link or selectable control."
                )

        historical_calendar = discover_calendar_week(
            driver,
            season_label="2019-2020",
            week_label="Week 9",
        )

        result["calendar_discovery_historical"] = historical_calendar

        if not historical_calendar["week_link"]:
            raise RuntimeError(
                "Could not find the rendered Week 9 calendar link after "
                "switching to the 2019-2020 season."
            )

        historical_url = historical_calendar["week_link"]["href"]
        print("Historical 2019-2020 Week 9 URL:", historical_url)
